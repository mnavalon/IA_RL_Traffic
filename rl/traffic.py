...
class TrafficEnv(gym.Env):
    def __init__(self, nlanes, ncars, images=True, sh=50):
        ...
        self.action_space = spaces.Tuple((
            spaces.Discrete(3),
            spaces.Box(self.l_lims, self.h_lims)
        ))
    ...
    def reset(self):
        ...
        return self.road_img(car0y - 0.5, self.sw, self.sh)[0] if self.images else self.get_state()
    ...
    def step(self, action):
        ...
        if not self.is_final:
            ...
            self.is_final = True
            reward = 0.0
        return rimg_i if self.images else self.get_state(), reward, int(self.is_final), {}
